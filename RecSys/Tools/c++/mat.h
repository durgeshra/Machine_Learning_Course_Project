...
public:
	...
	SMat(string fname, _int abc, _int def)
	{
		// cerr <<"Using Me Zipper" <<endl;
		contiguous = false;
		...
		// sprintf(cmd,"gunzip -c %s",fname.c_str());

		sprintf(cmd,"./comp -c %s",fname.c_str());
		...
		for(_int i=0; i<nc; i++)
		{
			...
			short int cols;
			fread(&cols,2,1,rf);
			//rf.read((char *) &cols, sizeof(short int));

			for(_int k = 0 ; k < cols ; k++)
			{
				short int ind;
				//rf.read((char *) &ind, sizeof(short int));
				fread(&ind,2,1,rf);
