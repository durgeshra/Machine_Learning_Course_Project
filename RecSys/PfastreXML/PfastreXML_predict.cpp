...
int main(int argc, char* argv[])
{
	...
	if(pfparam.pfswitch)
	{
		cerr << "prediction time: " << ((predict_time/tst_X_Xf->nc)*1000.0) << " ms/point" << endl;
		cerr << "model size: " << (model_size/1e+9) << " GB" << endl;
	...
	string w_file = model_folder + "/wb";
	check_valid_filename(w_file,true);
	SMatF* w_mat = new SMatF(w_file,1,1);
